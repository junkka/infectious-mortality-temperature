...
```{r}
h0 %>% ungroup() %>% 
  filter(date >= "1868-01-01", date < "1893-01-01") %>% 
  group_by(date) %>% summarise(temperature = mean(temp, na.rm = T)) %>% ungroup() %>% 
  summarise(min = min(temperature, na.rm=T), max = max(temperature, na.rm=T)) %>% 
  flextable() %>% 
  set_caption("Daily mean min, max")
```
...
ha1 <- harno %>% 
  mutate(
    t = case_when(
      as.character(time) %in% c("06:00:00", "07:00:00") ~"morning",
      as.character(time) %in% c("12:00:00", "13:00:00") ~"day",
      as.character(time) %in% c("18:00:00", "20:00:00") ~"evening",
    )
  ) 
...
d3 %>% 
  ...
  set_names(c(" ", "Births", "Mortality", "WFID", "AID", "Temperature")) %>% 
  flextable() %>% 
  autofit()
```
...
a1 %>% 
  group_by(dy) %>% 
  summarise(deaths= sum(dead), pop = sum(pop)) %>% 
  ungroup() %>% 
  mutate(r = (deaths/(pop))*1000) %>% 
  ggplot(aes(dy, r)) + 
  geom_hline(yintercept = 0, size = .2) +
  geom_line(color = "gray50") +
  scale_x_continuous(breaks = brks$start_day, labels = brks$month) +
  geom_smooth(se = F, span = .5) +
  labs(x = "Day of month", y = "Daily mortality rate") + 
  theme_classic()

ggsave("../figures/month_rate.png", height = 4, width = 7)
...
a1 %>% 
  group_by(dy) %>% 
  summarise(deaths= sum(dead), AID = sum(air), WFID = sum(water), Other = sum(unknown), pop = sum(pop)) %>% 
  ungroup() %>% 
  pivot_longer(cols = c(deaths:Other), names_to = "COD", values_to = "deaths") %>% 
  mutate(r = (deaths/(pop))*1000) %>% 
  group_by(COD) %>% 
  mutate(
    r = zoo::rollmean(r, 7, fill = NA, align = "center")
  ) %>% 
  filter(COD %in% c("AID", "WFID")) %>% 
  ggplot(aes(dy, r)) + 
  geom_hline(yintercept = 0, size = .2) +
  geom_line(color = "gray50") +
  scale_x_continuous(breaks = brks$start_day, labels = brks$month) +
  geom_smooth(se = F, span = .5, color ="#0F5298") +
  facet_wrap(~COD) +
  labs(x = "Day of month", y = "Daily mortality rate") + 
  theme_classic()

ggsave("../figures/month_rate_cod.png", height = 4, width = 7, scale = 1.3)
```
...
```{r}
expos <- d3 %>% select(temp1:temp20) %>% as.matrix() %>% as.vector() %>% na.omit()
...
pl_d <- d3 %>% 
  ...
  mutate(
    cause = factor(cause, levels = c("l", "a", "w"), labels = c("All-cause", "AID", "WFID"))
  ) 
...
pred_fun_lag_non <- function(x, exp, cent= NULL, min_temp = NULL, max_temp = NULL){
  ...
  # f
  fit4 <- glm(dead~  offset(log(pop)) + crb2+ ns(y, df = 3) + ns(dy, df = 3) + age_g + urban, data = x,
              family = "poisson")
...
```{r}
ra1 <- bind_rows(
  r2_14 %>% mutate(outcome = "AID"),
  r3_14 %>% mutate(outcome = "WFID"), 
  r4_14 %>% mutate(outcome = "Other")
) %>% 
  mutate(outcome = factor(outcome, levels = c("WFID", "AID", "Other")) %>% fct_rev) 
...
ggsave("../figures/urban-temp-cause-all.png", height = 3.3, width = 7)
```
...
ra1_a <- bind_rows(
  r2 %>% mutate(outcome = "AID"),
  r3 %>% mutate(outcome = "WFID"), 
...
perc_g <- d4 %>% 
  ...
  mutate(
    name = factor(
      name,
      levels = c("Air", "Dead", "Other", "Water"),
      labels = c("AID", "Dead", "Other", "WFID")
    )
  )
...
ra1_a %>% filter(predvar %in% c(-20, 20), lag > -1) %>% 
  ...
  mutate(cis = sprintf("%.2f - %.2f", rr_low, rr_high), rr = round(rr, 2)) %>% select(predvar, lag, outcome, rr, cis) %>% 
...
season_fun <- function(x, exp, cent= NULL){
  ...
  fit4 <- glm(dead~ offset(log(pop)) + age_g + crb2 + age_g + urban, data = x,
              family = "poisson")
...
season_fun_temp <- function(x, exp, cent= NULL){
  ...
  fit4 <- glm(dead~ ns(y, df = 3) + ns(lag_temp, df = 3) + age_g + ns(temp, df = 3) + offset(log(pop)) + crb2 + age_g + urban, data = x,
              family = "poisson")
...
```{r}
res1 %>%  left_join(distinct(t_brks), by = c("predvar" = "start_d")) %>% filter(!is.na(month), out != "all") %>% 
  mutate(cis = sprintf("%.3f - %.3f", rr_low, rr_high), rr = round(rr, 3)) %>% 
  select(month_n, out, rr, cis) %>% 
  pivot_wider(names_from = out, values_from = c(rr, cis)) %>% 
...
d4_p <- d3 %>% filter(!is.na(placebo_temp14))

temp_exps_p <- d4_p %>% select(placebo_temp1:placebo_temp20) %>% as.matrix
```

```{r}

temp_d <- d4_p$lag_temp[d4_p$dead > 0]
  
min_temp <- ceiling(min(pmin(temp_d), na.rm =T))
max_temp <- floor(max(pmax(temp_d), na.rm =T))


r0_p <- pred_fun_lag_non(d4_p, temp_exps_p, cent = 5, min_temp = min_temp, max_temp = max_temp)


ggplot(r0_p %>% filter(lag == -1), aes(predvar, rr)) + geom_hline(yintercept = 1) +
  geom_line(color = "#0F5298") +
  geom_ribbon(aes(ymin = rr_low, ymax = rr_high), alpha = .2, fill = "#0F5298") +
  scale_x_continuous(breaks = seq(-40, +25, 5)) + 
  # scale_y_log10(breaks = c(.4,.7,1,2,4,8,16)) +
...
temp_d <- d4_p$lag_temp[d4_p$air > 0]
  
min_temp <- ceiling(min(pmin(temp_d), na.rm=T))
max_temp <- floor(max(pmax(temp_d), na.rm=T))


r2_14_p <- pred_fun_lag(d4_p %>% mutate(dead = air), temp_exps_p, cent = 5, min_temp = min_temp, max_temp = max_temp)

r2_p <- pred_fun_lag(d4_p %>% mutate(dead = air), temp_exps_p, cent = 5)




ggplot(r2_14_p %>% filter(lag == -1), aes(predvar, rr)) + geom_hline(yintercept = 1) +
  geom_line() +
  geom_ribbon(aes(ymin = rr_low, ymax = rr_high), alpha = .2, color = NA) + 
  scale_x_continuous(breaks = seq(-40, +25, 5)) + 
  scale_y_log10(breaks = c(.4,.7,1,2,4,8,16)) +
...
temp_d <- d4_p$lag_temp[d4_p$water > 0]
  
min_temp <- ceiling(min(pmin(temp_d), na.rm = T))
max_temp <- floor(max(pmax(temp_d), na.rm = T))


r3_14_p <- pred_fun_lag(d4_p %>% mutate(dead = water), temp_exps_p, cent = 5, min_temp = min_temp, max_temp = max_temp)
r3_p <- pred_fun_lag(d4_p %>% mutate(dead = water), temp_exps_p, cent = 5)


ggplot(r3_14_p %>% filter(lag == -1), aes(predvar, rr)) + geom_hline(yintercept = 1) +
  geom_line() +
  geom_ribbon(aes(ymin = rr_low, ymax = rr_high), alpha = .2, color = NA) + 
  scale_x_continuous(breaks = seq(-40, +25, 5)) + 
  scale_y_log10(breaks = c(.4,.7,1,2,4,8,16)) +
...
d4_p <- d4_p %>% mutate(other = dead - water - air)

temp_d <- d4_p$lag_temp[d4_p$other > 0]
  
min_temp <- ceiling(min(pmin(temp_d), na.rm = T))
max_temp <- floor(max(pmax(temp_d), na.rm = T))


r4_14_p <- pred_fun_lag(d4_p %>% mutate(dead = dead - water - air), temp_exps_p, cent = 5, min_temp = min_temp, max_temp = max_temp)

r4_p <- pred_fun_lag(d4_p %>% mutate(dead = dead - water - air), temp_exps_p, cent = 5)


ggplot(r4_14_p %>% filter(lag == -1), aes(predvar, rr)) + geom_hline(yintercept = 1) +
  geom_line() +
  geom_ribbon(aes(ymin = rr_low, ymax = rr_high), alpha = .2, color = NA) + 
  scale_x_continuous(breaks = seq(-40, +25, 5)) + 
  scale_y_log10(breaks = c(.4,.7,1,2,4,8,16)) +
...
```{r}
ra1_p <- bind_rows(
  r2_14_p %>% mutate(outcome = "AID"),
  r3_14_p %>% mutate(outcome = "WFID"), 
  r4_14_p %>% mutate(outcome = "Other")
) %>% 
  mutate(outcome = factor(outcome, levels = c("WFID", "AID", "Other")) %>% fct_rev) 
...
ra1_p %>% 
  ...
  geom_ribbon(aes(ymin = rr_low, ymax = rr_high), alpha = .2, fill = "#0F5298", color = NA) + 
  scale_x_continuous(breaks = seq(-40, +25, 10)) + 
  scale_y_log10(breaks = c(.4,.7,1,2,4,8,16),limits = c(0.2, 9)) +
  theme_classic() +
  facet_wrap(~outcome) +
  theme(panel.grid.major = element_line(color = "gray90")) +
  labs(x = "Temperature", y = "IRR")

ggsave("../figures/placebo-temp-cause-all.png", height = 3.3, width = 7)
```
...
ra1_a_p <- bind_rows(
  r2_p %>% mutate(outcome = "AID"),
  r3_p %>% mutate(outcome = "WFID"), 
  r4_p %>% mutate(outcome = "Other")
) %>% 
  mutate(outcome = factor(outcome, levels = c("WFID", "AID", "Other")) %>% fct_rev) 


ra1_a_p %>% filter(predvar %in% c(-20,20), lag > -1) %>% 
  ggplot(aes(lag +1, rr)) + geom_hline(yintercept = 1) +
  geom_line(color = "#0F5298") +
  geom_ribbon(aes(ymin = rr_low, ymax = rr_high), alpha = .2, fill = "#0F5298", color = NA) +
...
```{r}
r0a_p <- pred_fun_lag_non(d4_p %>% mutate(dead = air), temp_exps_p, cent = 7)
r0w_p <- pred_fun_lag_non(d4_p %>% mutate(dead = water), temp_exps_p, cent = 5)
r0o_p <- pred_fun_lag_non(d4_p %>% mutate(dead = dead - air - water), temp_exps_p, cent = 5)


bind_rows(
  r0w_p %>% mutate(outcome = "Water"),
  r0a_p %>% mutate(outcome = "Air"),
  r0o_p %>% mutate(outcome = "Other")
) %>% 
  filter(lag == -1, rr < 5, rr > .4) %>%
  ggplot(aes(predvar, rr)) + geom_hline(yintercept = 1) +
  geom_line(color = "#0F5298") +
...
exposures <- temp_exps
  crb2 <- crossbasis(
    exposures, 
    lag = 13, 
    argvar =  list(fun = "thr", side = "d", thr.value =5) ,
    arglag = list(fun = "ns", knots = logknots(1:13, df = 4))
  ) 
  
fit4 <- glm(dead~  offset(log(pop)) + crb2+ ns(y, df = 3) + ns(dy, df = 3) + age_g + urban, data = d4,
              family = "poisson")
fit5 <- glm(dead~  offset(log(pop)) + crb2+ ns(y, df = 3) + ns(dy, df = 3) + age_g, data = d4,
              family = "poisson")
